# -*- coding: utf-8 -*-
"""
Created on Tue Mar 29 07:24:39 2022
...
def stagek(l1,minimum_support_count,k):
    keys=sorted([i[0] for i in l1.keys()])
    combi=list(combinations(keys,k))

    ck,lk={},{}
    for iter1 in combi:
        curr=l1[(iter1[0],)]
        for itm in iter1:
            curr=curr.intersection(l1[(itm,)])
        ck[iter1]=curr
...
max_freq_size=len(l1)
l_last=None

min_conf_perc = 0.5
for i in range(2,max_freq_size+1):
    ci,li=stagek(l1,min_supp,i)
